...
namespace icicle {
  ...
  template <typename S>
  eIcicleError ntt_cuda_init_domain(const Device& device, const S& primitive_root, const NTTInitDomainConfig& config)
  {
    ...
    auto err = ntt::Domain<S>::init_domain(primitive_root, device_context, fast_twiddles);
    return translateCudaError(err);
  }

  template <typename S>
  eIcicleError ntt_cuda_release_domain(const Device& device, const S& dummy)
  {
    using namespace device_context;
    DeviceContext device_context = get_default_device_context();
    device_context.device_id = device.id;
    auto err = ntt::Domain<S>::release_domain(device_context);
    return translateCudaError(err);
  }

  template <typename S, typename E>
  eIcicleError
  ntt_cuda(const Device& device, const E* input, int size, NTTDir dir, const NTTConfig<S>& config, E* output)
  {
  ...
  REGISTER_NTT_INIT_DOMAIN_BACKEND("CUDA", ntt_cuda_init_domain<scalar_t>)
  REGISTER_NTT_RELEASE_DOMAIN_BACKEND("CUDA", ntt_cuda_release_domain<scalar_t>);
