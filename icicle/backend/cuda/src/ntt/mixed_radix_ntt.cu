#include "ntt/mixed_radix_thread_level_ntt.cuh"
...
#include "icicle/ntt.h" // for Ordering
...
namespace mxntt {
    ...
    E* out_vec)
  {
    ...
    if (rev_type != eRevType::None) {
      ...
      // vec is now placed at index tid, which is the opposite of a DIT-digit-reverse --> this is the
      // DIF-digit-reverse. Therefore we use the DIF-digit-reverse to know which element moved to index tid and use
      // it to access the corresponding element in scalars vec.
...
#pragma unroll 1
    ...
    cudaStream_t cuda_stream)
  {
    ...
    if (log_size == 1 || log_size == 2 || log_size == 3 || log_size == 7) {
      THROW_ICICLE_ERR(eIcicleError::INVALID_ARGUMENT, "size not implemented for mixed-radix-NTT");
    }
    ...
    cudaStream_t cuda_stream)
  {
    ...
    if (NOF_BLOCKS != NOF_BLOCKS_64b) {
      THROW_ICICLE_ERR(eIcicleError::INVALID_ARGUMENT, "NTT dimensions (ntt_size, batch) are too large. Unsupported!");
    }
